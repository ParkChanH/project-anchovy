...
export default function Home() {
  const { user, profile, loading, isOffline, signIn } = useAuth();
  const dDay = calculateDDay();
  const todayLabel = getTodayLabel();

  // 자동 익명 로그인 (Firebase 설정이 있을 때만)
  useEffect(() => {
    if (!loading && !user && !isOffline) {
      signIn();
    }
  }, [loading, user, isOffline, signIn]);
  ...
  return (
    <main className="flex flex-col min-h-dvh safe-top safe-bottom">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-24">
        {/* 헤더 */}
        <Header dDay={dDay} todayLabel={todayLabel} isOffline={isOffline} />
...
// 헤더 컴포넌트
function Header({ dDay, todayLabel, isOffline }: { dDay: number; todayLabel: string; isOffline: boolean }) {
    ...
    <header className="flex justify-between items-start pt-4 pb-5 animate-fade-in">
      ...
      <div className="flex flex-col items-end gap-1.5">
        ...
        <div className="relative">
          <div className="absolute inset-0 bg-[#C6FF00]/20 rounded-full blur-md" />
          <div className="relative px-3 py-1.5 rounded-full bg-gradient-to-r from-[#2E7D32]/30 to-[#1a472a]/30 border border-[#2E7D32]/50">
